...
export function CreateCourse() {
  ...
  async function saveCourse(forReview: boolean) {
    ...
    let courseErr = await supabase
      .from('courses')
      .insert({ ...base, review_status: publishNow ? 'approved' : 'draft' })
      .select('id, published, review_status')
      .single()

    if (courseErr.error?.message?.includes('review_status')) {
      courseErr = await supabase.from('courses').insert(base).select('id, published, review_status').single()
    }

    if (
      publishNow &&
      courseErr.data?.id &&
      (!courseErr.data.published || courseErr.data.review_status === 'pending_review')
    ) {
      await supabase
        .from('courses')
        .update({ published: true, review_status: 'approved', review_note: null })
        .eq('id', courseErr.data.id)
    }
