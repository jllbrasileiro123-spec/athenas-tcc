...
type LearningCourse = Course & { completed_count: number }
...
export function MyCourses() {
  ...
  const [learning, setLearning] = useState<LearningCourse[]>([])
  ...
  const load = useCallback(async (silent = false) => {
    ...
    if (enrollData) {
      ...
      const lessonIds = list.flatMap((c) => (c.lessons ?? []).map((l) => l.id))
      ...
      setLearning(
        list.map((c) => ({
          ...c,
          completed_count: (c.lessons ?? []).filter((l) => completedIds.has(l.id)).length,
        }))
      )
    } else {
      setLearning([])
...
function LearningSection({
  ...
  return (
    <div className="grid sm:grid-cols-2 gap-5">
      {courses.map((course) => {
        const total = course.lesson_count ?? course.lessons?.length ?? 0
        const done = course.completed_count
        const cta =
          total > 0 && done >= total
