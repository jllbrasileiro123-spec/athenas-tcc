...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const applySession = useCallback((s: Session | null) => {
    ...
    // Não usa cache de role aqui: role (admin/instrutor) muda no SQL e o cache
  ...
  const fetchProfile = useCallback(async (userId: string) => {
    ...
    if (!error && data) {
      const p = { ...(data as object), phone: null } as Profile
    ...
    console.warn('[ATHENAS] falha ao carregar perfil', error?.message ?? 'sem dados')
    profileUserRef.current = null
    setProfile(null)
    clearProfileCache()
    return null
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (s?.user && (shouldRefetchProfile(event) || event === 'TOKEN_REFRESHED')) {
        const uid = s.user.id
        window.setTimeout(() => {
          if (!cancelled) void fetchProfile(uid)
        }, 0)
      }
