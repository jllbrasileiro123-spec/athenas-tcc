...
- Login social: Google (configurar no Supabase + Google Cloud)
...
3. Em seguida, execute `supabase/instructor-applications.sql` (solicitações de instrutor + revisão de cursos).
4. Depois, execute `supabase/gamification.sql` (trilha, XP, sequência, moedas e congelador).
5. Execute `supabase/quiz-questions.sql` (coluna review_status + perguntas de quiz).
6. Para virar admin (moderação): no SQL Editor,
   `update public.profiles set role = 'admin' where id = '<seu-user-uuid>';`
7. Em **Authentication → Providers → Email**:
...
supabase/
  ...
  gamification.sql     Trilha, XP, sequência, moedas
```
